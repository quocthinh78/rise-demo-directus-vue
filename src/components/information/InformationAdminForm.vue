<template>
    <div class="pt-8 px-8 container">
            ...
            <div class="mb-4 box-border">
                ...
                <div class="w-full bg-[var(--background-input)] flex flex-row rounded-md border border-solid"
                    ...
                    <input type="text" :disabled="disabledForm" id="contact-number" placeholder="Your phone"
                        class="bg-transparent p-2 text-sm rounded-lg h-10 outline-none border-none focus:border-transparent focus:ring-transparent disabled:bg-[var(--background-subdued)] disabled:text-[var(--foreground-subdued)] contact-number"
                        @focus="isPhoneFocus = true" @blur="isPhoneFocus = false" @keypress="onlyNumber"
                        v-model="phoneNumber" />
                </div>
        ...
        <div class="py-4">
            <button type="button"
                class="bg-[#1e74fd] focus:outline-none focus:ring-1 focus:ring-[#1e74fd] focus:border-[#1e74fd] text-[var(--v-button-color)] font-medium rounded-lg text-sm px-5 py-2.5 w-full items-center text-center disabled:bg-[var(--v-button-background-color-disabled)] disabled:cursor-default disabled:text-[var(--foreground-subdued)]"
    ...
    setup() {
        ...
        var submitForm = async () => {
            const data = {
                ...
                country: `${signupData.value.countryCode}(${countryCodeSelected.value.value})`,
                ...
                country_code: countryCodeSelected.value.value.replace('+', ''),
