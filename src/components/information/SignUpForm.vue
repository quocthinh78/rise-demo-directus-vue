<template>
    <div class="pt-8 px-8 container">
        ...
        <div id="alert-modal" tabindex="-1"
            class="fixed inset-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full justify-center items-center"
                ...
                <div class="relative bg-[var(--background-normal)] rounded-lg shadow">
                    <div class="p-6 space-y-6 flex-col ">
                        <div class="flex items-center justify-center">
                            <svg aria-hidden="true" class="mx-auto mb-4 text-white w-14 h-14" fill="bg-red-600"
                        ...
                        <div class="flex items-center justify-center">
                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">You need to do ekyc before
                                signing up</h3>
                        </div>
...
onMounted(async () => {
    const res = await AppApi("get", "/business-auth/business-role")
    if (res.data) {
...
const submitForm = async () => {
    ...
    if (resEKYC.value) {
        ...
        try {
            const res = await AppApi("post", "/wallex-business/signup", payload)
            if (res.data) {
                const data = {
                    firstName: firstName.value,
                    lastName: lastName.value,
            ...
            ToastRef.value.setValue({ message: msg || msg.message, status: "error" })
        }
    }
